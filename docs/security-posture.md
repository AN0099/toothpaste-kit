...
| Second party check on the markdown gate | Present, and it has caught a divergence | `markdownlint-cli2` runs the real tool against the same rule IDs as `scripts/mdlint.py`. On its first run, 2026-09-19, it found `MD034` at `CODE_OF_CONDUCT.md` line 52 where `mdlint.py` reports the file clean. Our rule fires on a whitespace delimited bare URL and not on one inside parentheses, and its planted positive at `mdlint.py:361` used the delimited shape, so the selftest proved the rule could fire rather than that it covered its field |
...
| Alerts outstanding on the release branch | **Ten, all in `scripts/mdlint.py`** | Seven `py/polynomial-redos` and three `py/path-injection`, all high security severity, from the first analysis to run against the release commits. The ReDoS findings are not dismissible on the same reasoning as the path ones: this linter parses contributor markdown inside a required check, so a pathological input can hang the gate. Unresolved, and the ruleset blocks the merge until they are |
