...
| 4.6 Checks actually binding | **Measured twice on 2026-09-19. The answer was no, and is now yes** | A workflow gates nothing until it is a required status check in the ruleset on the default branch. The first measurement found no `required_status_checks` rule, so `gates`, `markdownlint` and `check` ran on every push and bound nothing. The ruleset was changed twenty minutes after the commit carrying that finding merged, which is the only order available: a check cannot be required before it exists and has reported. Read back from the rulesets API after the change, rather than inferred from the request succeeding: ruleset `23662331` now carries `required_status_checks` binding five contexts, `gates`, `markdownlint`, `check`, `Analyze (python)` and `Analyze (actions)`, with `strict_required_status_checks_policy` false. The `CodeQL` status is deliberately not among them, because the `code_scanning` rule in the same ruleset already produces it and one rule waiting on another rule's output is a deadlock waiting for a rename. The bound names are **job** names rather than workflow names. This row stood unverified from the day it was written until a badge questionnaire forced the question, which is the argument for stating a check's coverage rather than its result |
...
| Control | State | Evidence |
|---|---|---|
| 6.1 Branch protection on the default branch | Present, as a ruleset | Not verifiable from the tree, and verifiable from the API, which is the distinction this row previously missed. Measured 2026-09-19: ruleset `23662331`, named `Default`, active, targeting `~DEFAULT_BRANCH`, carrying six rules: `deletion`, `non_fast_forward`, `code_scanning`, `code_quality`, `required_signatures` and `required_status_checks`. Branch deletion and force push are both blocked and `bypass_actors` is empty. **A direct push to the default branch cannot succeed at all**, established by attempting one: the `code_scanning` rule requires CodeQL results for the commit being pushed, and CodeQL cannot produce results for a commit the remote has refused to accept. Every change therefore arrives through a pull request, although no `pull_request` rule is present to say so, which is why nobody had written this down. It is a ruleset rather than a classic branch protection object, so a reader looking at the protection endpoint alone will find nothing and conclude wrongly |
