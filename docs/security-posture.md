...
| Signed releases | **Not configured** | No signing key configured for this repository. `docs/releasing.md` step 5 signs the tag if one is |
...
| Least-privilege workflow tokens | Partial | `gates.yml` declares `permissions: contents: read`. `commit-attribution.yml` declares none, so it inherits the repository default |
| Static analysis | **Absent** | No shell or Python linting. The tree is one Python script with no third-party imports and two shell scripts |
